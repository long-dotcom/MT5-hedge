...
`execution_mode=paper` 现在表示完整模拟执行：Hyperliquid 腿使用 NautilusTrader sandbox 和实时订单簿撮合，MT5 腿使用当前 MT5 demo 账户下单。开启前需要 `NAUTILUS_HYPERLIQUID_ENABLED=true`、`MT5_DEMO_ORDER_ENABLED=true`，并确保 MT5 `account_info().trade_mode` 是 demo。`MT5_LOGIN`、`MT5_PASSWORD`、`MT5_SERVER` 仍是唯一的 MT5 登录配置；如果配置了 `MT5_LOGIN` 或 `MT5_SERVER`，paper demo 下单前会要求当前账户 login/server 与它们一致，防止终端切到其他账户后误发单。`NAUTILUS_HYPERLIQUID_SIM_STARTING_BALANCES` 用逗号配置 sandbox 初始资金，例如 `100000 USD`。
...
Paper 完整模拟前可调用 `GET /api/settings/paper-readiness`。该检查不会发单，只验证 NautilusTrader sandbox 依赖、MT5 demo 下单开关、当前账户 demo 状态、`MT5_LOGIN`/`MT5_SERVER` 账户锁定、MT5 登录配置和启用品种映射。paper 开仓和平仓入口会执行同一套检查，存在 `block` 项时不会提交 sandbox 或 MT5 demo 订单，也不会回退到旧本地假成交。
