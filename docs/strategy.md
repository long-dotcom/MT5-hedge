...
6. 更新内存当前价差状态，供链路监控和 SSE 实时推送读取。
7. 当扣除成本后仍有利润时，同步当前内存候选池；价差回落或方向切换时，对应未执行机会会移出当前候选池。
8. 更新内存扫描状态，供链路监控和 SSE 实时推送直接读取；扫描热路径不再同步 upsert `spread_current`、`spread_direction_current` 或 `arbitrage_opportunities`。
9. 独立持久化调度器按 `SCAN_PERSIST_INTERVAL_MS` 从内存读取最新值，批量同步 `spread_direction_current` / `spread_current` / `arbitrage_opportunities`，并按 `SPREAD_BUCKET_SECONDS` 与 `SPREAD_HISTORY_INTERVAL_SECONDS` 写入 `spread_buckets` / `spread_snapshots`，供价差研究和分位数计算使用。
...
平仓订单提交后先写入内存状态和轻量执行结果队列，后台 `hedge_pool_persistence` worker 再异步把订单、成交、对冲组状态和事件落库。数据库仍然是恢复、历史和审计源；低延迟触发判断以运行期内存池为准。若落库失败，事件会重新放回队列等待下一轮重试。服务重启时会重新从数据库恢复对冲池，执行回查 worker 会继续修正 pending/closing 状态并同步内存。
