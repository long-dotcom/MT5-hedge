...
- `paper`：完整模拟执行。Hyperliquid 腿通过 NautilusTrader sandbox 使用实时订单簿模拟撮合；MT5 腿向当前 MT5 demo 账户提交 `order_send`，因此会在 demo 终端产生真实模拟订单。
...
Paper 模式不再回退到本地假成交。启用完整模拟前必须满足 `GET /api/settings/paper-readiness`：`NAUTILUS_HYPERLIQUID_ENABLED=true`，NautilusTrader Hyperliquid 与 sandbox 模块可导入，`MT5_DEMO_ORDER_ENABLED=true`，并且 MT5 `account_info().trade_mode` 是 demo。`MT5_LOGIN`、`MT5_PASSWORD`、`MT5_SERVER` 是唯一的 MT5 登录配置；如果配置了 `MT5_LOGIN` 或 `MT5_SERVER`，paper demo 下单前会要求当前账户 login/server 与它们一致。任一检查失败时，paper 开仓和平仓会直接拒绝执行，不会使用旧 `PaperAdapter` 假成交。

执行层已经接入 NautilusTrader Hyperliquid 边界：上层对冲组管理只创建双腿意图并维护 hedge lifecycle；单腿订单提交先经过 `ExecutionGateway`，由 gateway 输出统一的 `OrderEvent` 和 `FillEvent`，再写入 `orders` / `fills`。当 Hyperliquid adapter 被标记为 simulated 时，`build_execution_gateway()` 会切换到 `NautilusHyperliquidSandboxGateway`；live 且 `NAUTILUS_HYPERLIQUID_ENABLED=true` 时切换到 `NautilusHyperliquidGateway`。MT5 腿仍由现有 adapter 执行，不由 NautilusTrader 接管。

NautilusTrader Hyperliquid gateway 不改变对冲组语义。它会把内部 `BTC` 等标准永续符号转换成 NautilusTrader instrument id，例如 `BTC-USD-PERP.HYPERLIQUID`；`xyz:JP225` 这类 HIP-3 DEX 符号会转换成 `xyz:JP225-USD-PERP.HYPERLIQUID`，行情侧通过 Nautilus `HyperliquidAllDexsAssetCtxs` custom data 接入。执行侧真实提交前仍需要先用对应品种做 testnet 或小额主网验证。打开真实提交开关后，gateway 会启动 NautilusTrader TradingNode，注册 bridge Strategy，并把 market/limit 单提交给 NautilusTrader；accepted/filled/rejected 事件会映射回系统订单结果。账户仓位和订单回查会跟随 `NAUTILUS_HYPERLIQUID_ENVIRONMENT` 查询 mainnet/testnet，仓位页展示的是交易所实际返回并同步入库的 live positions。真实运行前必须先安装 `nautilus_trader`，并配置 testnet 或小额主网凭证。
...
当前 Paper 模式的 Hyperliquid post-only、market/limit 成交由 NautilusTrader sandbox 按订单簿处理；MT5 侧由 broker demo 账户返回成交结果。
...
4. 当 `close_spread <= exit_target` 且 `estimated_profit >= auto_close_min_profit` 时，系统用反向订单同时平掉两边；paper 组走 Nautilus sandbox + MT5 demo 完整模拟，live 组只有在 `auto_close_live_enabled=true` 且 `live_trading_enabled=true` 时才提交实盘平仓腿。
...
手工关闭对冲组会按 `execution_mode` 执行反向订单：`paper` 走 Nautilus sandbox + MT5 demo 完整模拟，`live` 要求 `live_trading_enabled=true`，并分别通过 NautilusTrader Hyperliquid gateway 和 MT5Adapter 提交平仓腿。平仓腿会带 `reduce_only` 语义；MT5Adapter 在 hedging 账户下会先用 `positions_get(symbol=...)` 找到被减仓方向的持仓 ticket，并在 `order_send` 请求里写入 `position`，找不到可平仓持仓或请求数量超过当前持仓数量时拒绝发单，避免反向单变成新增仓位。双边成交后状态变为 `closed`；任一边只返回 `accepted/submitted` 且未成交时，状态保持 `closing`，不会把待成交订单当成已平仓；单边成交会进入 `manual_intervention`。
