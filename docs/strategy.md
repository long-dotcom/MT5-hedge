...
扫描器按四层闸门组织，避免把阻断条件、候选条件和风险提示混在同一个 `reason` 中：

1. `Market Gate`：先判断品种是否具备交易资格，包括 MT5 是否可报价、报价是否同步、数量换算是否有效、当前方向是否允许开仓、MT5 交易能力缓存是否有效。失败时记录 `gate=market/quote` 和 `blocker`，不会生成 `executable`。
2. `Signal Gate`：只判断价差、成本、利润和统计入场线。最终入场线为 `max(reachable_entry, min_entry_spread)`；价差未达到时是 `candidate`，覆盖成本和利润并达到入场线时才可能是 `executable`。
3. `Risk Tags`：只打标签，不阻断。当前包括 `overheat`，用于标记价差超过历史 p99 的极端机会，供链路监控、审计和后续排序使用。
4. `Liquidity Gate`：检查 Hyperliquid L2 或顶层深度。深度不足时从 `executable` 降级为 `candidate`，因为机会存在但当前盘口不可按目标数量成交。
5. 组合后的扫描结果写入内存当前价差、双向价差和机会池；价差回落或方向切换时，对应未执行机会会移出当前候选池。
6. 更新内存扫描状态，供链路监控和 SSE 实时推送直接读取；扫描热路径不再同步 upsert `spread_current`、`spread_direction_current` 或 `arbitrage_opportunities`。
7. 独立持久化调度器按 `SCAN_PERSIST_INTERVAL_MS` 从内存读取最新值，批量同步 `spread_direction_current` / `spread_current` / `arbitrage_opportunities`，并按 `SPREAD_BUCKET_SECONDS` 与 `SPREAD_HISTORY_INTERVAL_SECONDS` 写入 `spread_buckets` / `spread_snapshots`，供价差研究和分位数计算使用。

扫描频率和历史落库频率分离：

- `SCANNER_INTERVAL_MS`：大于 0 时启用毫秒级扫描，例如 100 表示 100ms 一次。
- `SCANNER_INTERVAL_SECONDS`：未设置毫秒扫描时使用的秒级扫描间隔。
- `SCAN_PERSIST_INTERVAL_MS`：扫描结果从内存同步到数据库的间隔，默认 1000ms；该任务独立运行，不阻塞下一轮扫描。
- `EXECUTION_MAINTENANCE_INTERVAL_MS`：自动执行、自动平仓和执行回查的维护周期，默认 1000ms；这些数据库任务不再挂在扫描热路径后面。
- `CARRY_COST_SYNC_INTERVAL_SECONDS`：资金费/过夜费同步周期，默认 300 秒；该任务已从执行维护循环拆出，不再每秒运行。
...
前端链路监控通过 SSE 接收最新状态，不再依赖页面定时轮询；SSE 请求使用 `Authorization: Bearer <token>` 请求头鉴权，避免 access token 出现在 URL、代理访问日志或浏览器历史中。账户信息并入仪表盘账户区展示。扫描和候选机会实时视图已合并到链路监控，优先读取内存扫描状态，服务刚启动或扫描尚未完成时才回退数据库当前表。价差研究只接收 bucket 变更信号后重拉当前图表数据。
...
前端对冲池采用按生命周期分组的固定高度滚动列表，页面只展示池内摘要、候选入口、平仓闸门和各状态订单行，避免对冲组数量增加后把链路监控整体布局拉长。

前端表格型页面尽量把长数据限制在卡片内部滚动，保留页面标题、筛选器和关键操作区稳定可见，避免订单、日志、风险事件、仓位或设置映射数量增加后撑长整页。

设置页策略参数和风控参数采用分组展开布局，宽屏下横向展示核心项和高级项，避免高级参数被折叠隐藏或表单只占用左侧窄栏。

## 对冲池与自动平仓

运行期会维护内存 `HedgePoolStore`，服务启动时从数据库加载 `opening/open/open_partial/closing/manual_intervention` 对冲组。开仓、手工标记、执行回查、资金费/隔夜费同步和旧 DB 执行路径完成后，都会把最新状态同步回内存池。

自动平仓不再每轮查询 `hedge_groups` 表。`run_auto_close()` 只读取内存对冲池中的 `open/open_partial` 快照，再读取 `QuoteCache`/`QuoteSynchronizer` 计算当前 `close_spread` 和估算利润。满足条件后先对内存状态执行 CAS：

```text
open/open_partial -> closing
```

只有 CAS 成功的线程才允许提交平仓订单，避免自动平仓和手工平仓重复触发。同一个手工平仓 API 也走这套内存状态机；如果对冲组不在运行池中，会直接返回“对冲组不在运行池中”，不会再用 DB 查询补救。

平仓触发路径中的退出线规则：

- 如果对冲组已有 `exit_target`，直接使用该值。
- 如果老对冲组缺少 `exit_target`，只用内存品种映射里的 `max_close_spread` 兜底。
- 自动平仓热路径不再调用历史价差查询，也不会为了分位数实时查库。

平仓订单提交后先写入内存状态和轻量执行结果队列，后台 `hedge_pool_persistence` worker 再异步把订单、成交、对冲组状态和事件落库。数据库仍然是恢复、历史和审计源；低延迟触发判断以运行期内存池为准。若落库失败，事件会重新放回队列等待下一轮重试。服务重启时会重新从数据库恢复对冲池，执行回查 worker 会继续修正 pending/closing 状态并同步内存；同步时会保留运行期已计算的 open/open_partial 未实现盈亏，避免用数据库旧值覆盖实时显示。

当前内存化自动平仓首版只执行 `paper` 对冲组。`live` 对冲组即使开启 `auto_close_live_enabled` 也会被跳过并记录日志，后续扩展 live 路径前需要补齐跨进程锁和更严格的外部订单回查。
...
- 统计入场线：默认用最近 `1h` 样本，`reachable_entry = max(p75(spread), mean + 1σ)`。每个品种还可以配置 `min_entry_spread`（前端显示为“最小买入价差”），最终入场线取 `max(reachable_entry, min_entry_spread)`；当前价差达到最终入场线、覆盖 `p90(cost)` 成本保护线，并满足最小总利润后，才进入 `executable`。`overheat` 使用入场价差分布的 p99 作为风险标记和审计字段，不再作为阻断入场的硬上限，避免品种级最小入场线高于 p99 时形成没有任何可执行窗口的配置冲突。
- 统计退出线：默认使用同方向 `close_spread` 的低分位回落目标，并用开仓价差反推利润保护上限：`exit_target = min(p25(close_spread), entry_spread - p90(unit_cost) - 每份利润缓冲)`。每个品种还可以配置 `max_close_spread`（前端显示为“最大卖出价差”），最终退出线在有统计线时取 `min(exit_target, max_close_spread)`，缺少统计线时可直接使用 `max_close_spread` 兜底；`0` 表示不启用品种级卖出价差上限，负数是有效上限。每份利润缓冲默认 `0`，跨品种利润保护优先使用 USD 口径的 `auto_close_min_profit`。当利润保护上限小于等于 0 且没有品种退出线上限时，退出线记为 0，表示当前参数下不能生成有效自动平仓线。开仓时会把 `trigger_spread`、触发时 HL/MT5 bid/ask、`entry_spread`、最终 `entry_threshold`、最终 `exit_target` 和 `overheat` 固定保存到对冲组；双边成交后 `entry_spread` 会被真实成交价差覆盖，`trigger_spread` 和触发盘口保留机会触发时的原始状态，方便对比执行质量。
...
系统新增“报价时差分析”页面，用于观察同一个内部品种中 Hyperliquid 与 MT5 谁先发生明显跳动，另一边是否在指定时间窗口内跟随。
...
- 顶部筛选栏控制品种、窗口、跳动阈值、最小跳动和最大跟随毫秒。
- 主区域展示标准化 mid 曲线和两边 mid 差，并在标题区显示 HL/MT5 最新 mid。
- 右侧合并展示 HL -> MT5 与 MT5 -> HL 两个方向的平均滞后、P90 滞后、跟随率和样本数。
- 底部跳动事件表展示领先方、跟随方、方向、滞后毫秒、跳动幅度、期间最大 mid 差。
...
- `paper`：纸面账本执行。默认 Hyperliquid 腿使用本地 `QuoteCache` 的最新 bid/ask 撮合；MT5 腿向当前 MT5 demo 账户提交 `order_send`，因此会在 demo 终端产生真实模拟订单。设置页开启“Paper 使用真实探针”后，所有非 MT5 腿会按 venue 启用通用探针标记；当前只有 Hyperliquid adapter 已实现真实探针下单，Nautilus venue 仍会拒绝下单。
- `live`：进入实盘路径，但必须先开启实盘开关。

真实下单路径仍需要显式开关保护。MT5 实盘下单需要 `MT5_LIVE_ORDER_ENABLED=true`；Hyperliquid `execution_mode=live` 下单当前仍固定阻止。paper-live 探针单只在 `execution_mode=paper` 且设置页或 env 兜底启用探针时进入真实探针路径，仍会保留 paper 对冲组账本语义。
...
执行层通过 `ExecutionGateway` 输出统一的 `OrderEvent` 和 `FillEvent`，再写入 `orders` / `fills`。`build_execution_gateway()` 当前统一返回 `AdapterExecutionGateway`；Hyperliquid paper 默认由本地 adapter 用最新报价撮合，开启 paper-live 探针后由真实最小量订单提供成交均价。Binance Futures 已接入同一套通用探针框架，具体下单由 Nautilus Binance HTTP client 完成，MT5 腿仍由现有 adapter 执行。
...
当 `execution_mode=paper`、某条非 MT5 腿启用 paper-live 探针且 `PAPER_LIVE_PARALLEL_EXECUTION=true` 时，系统在严格行情复核通过后会并发提交最小真实探针单和 MT5 demo 单，不再等待探针腿回执后才提交 MT5。两边回执返回后再写入 `orders` / `fills`。如果只有一边成交，会立即对已成交腿提交反向冲销：开仓阶段使用 reduce-only 平掉新增腿；平仓阶段反向恢复已平掉的腿，并记录 `parallel_single_leg_compensation` 事件。
...
开启 Hyperliquid paper-live 后，Hyperliquid paper 腿不再使用本地 L2 模拟价格，而是读取交易所 `meta` 中该资产 `szDecimals` 和 `allMids`，按 `max(10^-szDecimals, HYPERLIQUID_DEFAULT_MIN_NOTIONAL / mid)` 向上取整到数量步进，提交最小真实探针单。Binance Futures paper-live 使用 Nautilus 读取 `exchangeInfo`，按 `MARKET_LOT_SIZE` / `LOT_SIZE` 步进、最小数量和 `MIN_NOTIONAL` 计算最小探针数量，再发 `MARKET` 真实单取 `RESULT` 回执。开仓探针按 paper 腿方向提交；平仓探针尽量带 reduce-only。探针真实成交量只用于产生价格和外部订单号，写入 `fills` 的数量仍是策略目标数量，避免 MT5 补腿比例、价差和 PnL 被探针数量缩小。其他交易所需要在通用探针能力表中注册最小量计算、真实订单提交和成交回执解析后，才能真正发探针单。
...
候选机会默认不能直接下单，只有 `executable` 状态允许执行。人工强制下单入口可用当前方向快照创建一次性机会，并跳过策略入场线、价差和净利润闸门；它仍然会进入同一执行引擎，继续执行严格同步报价、MT5 session、MT5 订单预检查、readiness 和风控。执行引擎会再次读取严格同步报价：
