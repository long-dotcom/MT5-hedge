...
- 统计退出线：默认使用同一窗口的低分位回落目标，并叠加成本保护：`exit_target = max(p25(spread), p90(cost) + 每份利润缓冲)`。开仓时会把 `entry_spread`、`reachable_entry`、`exit_target` 和 `overheat` 固定保存到对冲组，避免持仓过程中目标线随滚动窗口频繁漂移。
...
首版真实 SDK 下单调用仍处于保护状态，需要在确认券商、主网、合约规格和回报机制后补齐。
...
- `single_leg_action`：单腿异常时进入人工介入或自动回滚。
...
4. 当 `close_spread <= exit_target` 且 `estimated_profit >= auto_close_min_profit` 时，系统用 paper 反向订单同时平掉两边。
5. 如果超过 `max_holding_minutes` 且利润达标，也允许按时间退出。

相关参数：

- `auto_close_enabled`：是否启用 Paper 自动平仓。
- `exit_target_percentile`：退出线低分位数，默认 `0.25`，用于寻找更低的回落目标。
- `auto_close_unit_profit_buffer`：每份平仓利润缓冲，默认 `20`；退出线不会低于 `成本保护线 + 缓冲`。
- `auto_close_min_profit`：自动平仓最低估算利润，默认 `0`。
- `max_holding_minutes`：最大持仓时间，超时且利润达标时允许退出。

自动平仓成功后，对冲组会写入 `close_reason`、`realized_pnl`，并记录两条反向订单和成交。单腿平仓异常会进入 `manual_intervention`。
